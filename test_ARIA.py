...
for mark in mark_sql_list.keys():
    ...
    for i, sql in enumerate(sql_list):
        ...
        all_results, support_rs = load_pre_query_results(sql, mark, i, db, r_support)
        print(i, sql, len(all_results))
        start_time = time.time()
        for _ in range(repeat_num):
            price = pricer.pre_price_SQL_query(sql, all_results, support_rs)
        end_time = time.time()
        # print(sql,price)
